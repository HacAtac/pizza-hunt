...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    ...
    .catch((err) => {
      ...
      window.history.back(); //new method that takes you back one page
    });
}
...
function handleNewCommentSubmit(event) {
  ...
  fetch(`/api/comments/${pizzaId}`, {
    ...
    .then((commentResponse) => {
      ...
      location.reload(); //this is a new method that reloads the page without a full page refresh (which is the default)
    })
    .catch((err) => {
      console.log(err);
      alert("Cannot add a comment! Please try again.");
    });
}
...
function handleNewReplySubmit(event) {
  ...
  fetch(`/api/comments/${pizzaId}/${commentId}`, {
    ...
    .catch((err) => {
      console.log(err);
      alert("Cannot add a reply! Please try again.");
    });
}
